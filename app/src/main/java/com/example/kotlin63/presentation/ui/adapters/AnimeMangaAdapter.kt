...
class AnimeMangaAdapter : PagingDataAdapter<Data, AnimeMangaAdapter.ViewHolder>(diffUtil) {

    inner class ViewHolder(private val binding: ItemAnimeMangaBinding) :
        RecyclerView.ViewHolder(binding.root) {

        fun onBind(animeData: Data) = with(binding) {
            Glide.with(ivItemAnimeManga.context).load(animeData.attributes.posterImage.original)
